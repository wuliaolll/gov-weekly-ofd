...
1. **空格分隔标题快速路径**：若标题含空格且**所有片段均不超过 18 字符宽**，按空格拆为短语片段后，通过递归枚举合并方案取最少行、行宽最均匀的方案。合并后的行内保留原始空格。若任意片段超宽，则跳过此路径落入 jieba DP。
2. **jieba 语义 DP 路径**（片段超宽或无空格标题统一走此路径）：
...
4. 标题智能折行：空格分隔快速路径（所有片段 ≤18 字符宽）+ jieba 语义 DP（片段超宽或无空格标题），两路径均正确运行，jieba DP 不再被绕过。
5. 标题中英文混排：DOCX 和 OFD 标题中的数字与英文字母均使用 Times New Roman 字体。
6. 标题隐藏字符清理（`_clean_title_text()`）：去除 HTML 结构噪声换行（`\n`/`\r` 等 6 种变体）、Zs 类 Unicode 空格归一化、圆圈占位符（°/○ 等）、零宽/BiDi 控制字符，消除字体缺字形显示为圆圈的问题。
7. DOCX 标题多段落结构：每行独立 `<w:p>` 段落而非 `<w:br/>` 软换行，Word 中段落结束符显示为 ¶（←）而非软换行符（↵），符合公文排版规范。
8. `scraper.py` 标题源头清理：`fetch_article_content()` 在返回前对 `title` 做 Zs 归一化与 Cc/Cf 字符过滤，减少后续处理中的异常字符。
